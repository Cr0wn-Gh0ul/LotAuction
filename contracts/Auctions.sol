...
contract AttributeAuction is ERC721, Ownable {
    ...
    // enum to define what type of auction this is
    enum LotType {
        PHASE_1,
        PHASE_2,
        PHASE_3,
    ...
    mapping(uint256 => mapping(uint256 => address)) public amountToAddress;
    ...
    mapping(uint256 => RedBlackTreeLibrary.Tree) public bidTree;
    ...
    /**
     * @dev Checks if the msg.sender is an active bidder
     */
    modifier activeBidder() {
        require(
            addressToAmount[auctionLive][msg.sender] > 0,
    ...
    function increaseBid() external payable notPaused activeBidder {
        ...
        uint256 lastBid = addressToAmount[auctionLive][msg.sender];
    ...
    function removeBid() external payable notPaused activeBidder {
        ...
        bidTree[auctionLive].remove(lastBid);
        // send lowest winning bid back to bidder
        _refundBid(amountToAddress[auctionLive][lastBid], lastBid);
    ...
    function settleAuction() external notPaused {
        ...
        (, , uint256 totalPrizes) = _getLotInfo(auction.lotType);
        // get winners of this auction
        address[] memory winners = _winnersOfAuction(auctionLive);
        ...
        prizesPerAddress[auctionLive] = totalPrizes / winners.length;
        // update auction info
        auctions[auctionLive] = auction;
    ...
    function setMinBidIncrementPercentage(uint256 minBidIncrementPercentage_)
        external
        onlyOwner
    {
    ...
    function addBid() public payable notPaused {
        ...
        if (lowestBid > 0) {
            // remove lowest winning bid from bid tree
            bidTree[auctionLive].remove(lowestBid);
            // send lowest winning bid back to bidder
            _refundBid(amountToAddress[auctionLive][lowestBid], lowestBid);
        } else {
            // increment number of winning bids
            auction.winningBidsPlaced++;
        }
        // add bid to active bid mapping
        addressToAmount[auctionLive][msg.sender] = msg.value;
        // add bid to bid tree
        bidTree[auctionLive].insert(msg.value);
        // for reverse tree lookup
        amountToAddress[auctionLive][msg.value] = msg.sender;
        // update auction info
        auctions[auctionLive] = auction;
    }
     ...
     */
    function checkActiveAuction(Auction memory auction_) public view {
        // check if the current auction has been settled
        require(!auction_.settled, "Auction has been settled");
        // check if the auction has started
        require(auction_.startTime >= block.number, "Auction has not started");
        // check if the auction has ended
        require(auction_.endTime <= block.number, "Auction has ended");
    }
    ...
    function validateBid(
        ...
        require(addressToAmount[auctionLive][addr_] == 0, "Bid already exists");
        ...
        require(!bidTree[auctionLive].exists(amount_), "Bid already exists");
    ...
    function _refundBid(address user_, uint256 amount_) internal {
        ...
        delete amountToAddress[auctionLive][amount_];
        // refund bid
    ...
    function _mint(
        ...
        (uint256 tokenIdLow, , ) = _tokenIdRange(auctionId_);
     ...
     * @param auctionId_ The id of the auction to get winners for
     */
    function _winnersOfAuction(uint256 auctionId_)
        internal
        view
     ...
     * @param auctionId_ The id of the auction to get the token id range for
     ...
     */
    function _tokenIdRange(uint256 auctionId_)
        internal
        view
        returns (
            uint256,
     ...
     */
    function _getAuctionLotType() internal view returns (LotType lotType) {
